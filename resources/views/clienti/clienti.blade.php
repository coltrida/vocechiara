            ...
            function () {       //tutto ciò che viene effettuato qui dentro lo farò a DOM pronto
                ...
                $('#clients_table').DataTable({        //----------------- lettura dati datatable
                    ...
                    columns: [
                        ...
                        {data: 'ultimav', name: 'ultimav', orderable:false, searchable:false}

                    ],
                    initComplete: function () {
                        this.api().columns([1,2,5,6,7,8,9,10,11]).every(function () {
                            var column = this;
                ...
                $('#clients_table').on('click', 'a.phone', function (ele) {
                   // ele.preventDefault();
                    ...
                    var prelinkcli = '{{ route('clients.phone', ":id") }}';
                    var linkcli = prelinkcli.replace(':id', idClient);
                    $.ajax(linkcli,
                        {
                            method: 'GET',
                            complete : function (resp) {
                                //console.log(resp.responseJSON);
                                var recall = resp.responseJSON.recall;
                                if (recall = 'x'){
                                    $('#recall').val(resp.responseJSON.datarecall);
                                    $('#nomeRecall').html('Recall '+resp.responseJSON.nome+' '+resp.responseJSON.cognome)
                                }
